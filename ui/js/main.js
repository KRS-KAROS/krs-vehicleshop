...
$(window).ready(function() {
    window.addEventListener('message', (event) => {
        ...
        if (data.action == "load"){
            ...
            intv = setInterval(() => {
                curTest = curTest - 1;
                $("#delivery-time").html("Test Di Guida <br/> "+curTest+" secondi rimasti");
            }, 1000);
...
function init(data) {
   ...
   if(garage.bank == true) {
	   ...
	   $("#btn-buy-bank").css("display","unset");
       $("#vehicle-price-money").css("display", "none");
       $("#vehicle-price-bank").css("display", "unset");
   }else {
	   $("#btn-buy-money").css("display","unset");
	   $("#btn-buy-bank").css("display","none");
       $("#vehicle-price-money").css("display", "unset");
       $("#vehicle-price-bank").css("display", "none");
   }
...
function loadCat(x) {
   ...
   $.each(garage.Vehicles[carCategories[x]], function (key, val) {
	   ...
	   if(key == 0) {
		  active = `active`;
		  $.post("https://krs-vehicleshop/showCar", JSON.stringify({ name: val.name }), function(x){});
          setPrice(val.price);
...
$(document).on('click', '.vehicle-list-name', function(e){
    ...
    $.post("https://krs-vehicleshop/showCar", JSON.stringify({ name: $(this).data("name") }), function(x){});
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: 
            $("#wrap").hide();
            $("body").hide();
			$.post("https://krs-vehicleshop/close", JSON.stringify({}), function(x){});
            break;
    }
});
...
function testdrive(){
            $("#wrap").hide();
            $("body").hide();
			$.post("https://krs-vehicleshop/testdrive", JSON.stringify({}), function(x){});
}
...
async function moveRight(){
    ...
    while (startMovingRight){
        $.post('https://krs-vehicleshop/moveright');
        await sleep(5);
    }
...
async function moveLeft(){
    ...
    while (startMovingLeft){
        $.post('https://krs-vehicleshop/moveleft');
        await sleep(5);
    }
...
$(document).on('click', '.choose-color-circle', function(e){
    ...
    $.post("https://krs-vehicleshop/setcolour", JSON.stringify({rgb: rgb}), function(x){});
});
...
function setPrice(x) { 
    $("#vehicle-price-bank").html(`<i class="fas fa-money-check-alt"></i>`+x);
    $("#vehicle-price-money").html(`<i class="fas fa-dollar-sign"></i>`+x);
}
...
function plakaonay() { 
    ...
    $.post("https://krs-vehicleshop/checkPlatePrice", JSON.stringify({plate: plate}), function(x){
...
function buy() {
   ...
   $.post("https://krs-vehicleshop/buy", JSON.stringify({bank: bank}), function(x){ 
        if(x == true) {  
            $("#wrap").hide();
            $("body").hide();
			//$.post("https://krs-vehicleshop/close", JSON.stringify({}), function(x){});
            ...
            $("#btn-buy-bank").animate({ color: "white", backgroundColor: "rgb( 255, 0, 0 )" });
            setTimeout(() => {
                $("#btn-buy-money").animate({ color: "white",  backgroundColor: "rgb( 115, 218, 144 )" });
                $("#btn-buy-bank").animate({ color: "white", backgroundColor: "rgb( 115, 218, 144 )" });
            }, 1000);
        }
    }); 
}
...
$(document).ready(function() {
    $('.colorpicker').minimalColorpicker({
        ...
        onUpdateColor: function(e, color) {
            $.post("https://krs-vehicleshop/setcolour", JSON.stringify({rgb: color.rgb}), function(x){});
        }
    });
});
